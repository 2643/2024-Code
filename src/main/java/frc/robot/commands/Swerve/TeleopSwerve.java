...
public class TeleopSwerve extends Command {
  ...
  public void execute() {
    ...
    if (RobotContainer.operatorBoard.getPOV() == 0) {
      rotationSup = () -> -1 ;
    } else if (RobotContainer.operatorBoard.getPOV() == 180) {
      rotationSup = () -> 1;
    }

    System.out.println(RobotContainer.operatorBoard.getPOV());
    ...
    if (RobotContainer.boostButton.getAsBoolean())
      placeholder = 4.5;
    else
      placeholder = 1.125;

    RobotContainer.s_Swerve.drive(
        new Translation2d(translationVal, strafeVal).times(placeholder),
        rotationVal,
